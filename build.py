...
<article class="card unit-card" id="u-{esc(u)}" data-unit="{esc(u)}">
  ...
  <div class="phases">{panels}</div>
    ...
    masthead = theme.masthead(
        ...
        "What each unit cost in time &mdash; manual QA and MEP snagging onsite, "
        "against photo capture and CONXAI processing. One card per unit.",
        f"Source <b>Damac snagging schedules &mdash; Activity logs</b><br>"
        f"Units <b>{len(units)}</b> &middot; Generated <b>{GENERATED:%d %b %Y}</b><br>"
        f"Times as recorded onsite (GST)")
...
.tile .k{{display:block;font-size:10.5px;text-transform:uppercase;letter-spacing:.07em;
  color:var(--muted);font-weight:650;line-height:1.35;min-height:2.7em}}
